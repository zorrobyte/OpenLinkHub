...
// Package: Razer Keyboard Driver
// Supports Razer keyboards with the extended matrix protocol (class 0x0F).
...
var (
	...
	colorPacketLength     = 3 // Single zone: whole keyboard
...
// Init initializes the Razer keyboard device
func Init(vendorId, productId uint16, serial, path string) *common.Device {
	pwd = config.GetConfig().ConfigPath

	// Look up device in the Razer device table
	...
	d.getSerial()
	if len(d.Serial) == 0 {
		// If the serial passed from enumeration is a path, use product ID instead
...
// createDevice registers the device with the common device registry
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeRazerKeyboard,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
	...
	if d.DeviceProfile == nil {
		...
		deviceProfile.Stand = &Stand{
			Row: map[int]Row{
				1: {
					Zones: map[int]Zones{
						1: {Name: "Keyboard", Width: 400, Height: 120, Left: 0, Top: 0, PacketIndex: []int{0}, Color: rgb.Color{Red: 0, Green: 255, Blue: 255, Brightness: 1}},
					},
				},
			},
		}
	...
	go func(lightChannels int) {
		...
		for {
			...
			default:
				...
				// For animated modes, use the first 3 bytes (single zone: whole keyboard)
				if len(buff) >= 3 {
