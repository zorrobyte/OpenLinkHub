...
// Package: Razer Kraken Headset Driver
// Supports Razer Kraken headsets with the 37-byte Kraken HID protocol.
...
type Device struct {
	...
	krakenCfg       razer.KrakenDeviceConfig
}
...
// Init initializes the Razer Kraken headset device
func Init(vendorId, productId uint16, serial, path string) *common.Device {
	...
	krakenCfg, ok := razer.KrakenConfigs[productId]
	if !ok {
		logger.Log(logger.Fields{"productId": productId}).Error("Razer Kraken config not found for PID")
		return nil
	}
	...
	d.instance = &common.Device{
		...
		Instance:    d,
		DeviceType:  common.DeviceTypeHeadset,
	}
...
// Stop stops all device operations
func (d *Device) Stop() {
	d.Exit = true
	...
	// Turn off LEDs
	_ = razer.KrakenSetModeNone(d.dev, d.krakenCfg)
...
func (d *Device) getSerial() {
	...
	if err != nil {
		...
		krakenSerial, krakenErr := razer.KrakenGetSerial(d.dev)
		if krakenErr != nil {
			logger.Log(logger.Fields{"error": krakenErr}).Warn("Unable to get device serial number from Kraken protocol")
			return
		}
		d.Serial = krakenSerial
		return
	}
	d.Serial = serial
}
...
func (d *Device) getDeviceFirmware() {
	fw, err := razer.KrakenGetFirmware(d.dev)
	if err != nil {
	...
	if d.DeviceProfile == nil {
		...
		deviceProfile.Stand = &Stand{
			Row: map[int]Row{
				1: {
					Zones: map[int]Zones{
						1: {Name: d.razerDev.Name, Width: 200, Height: 80, Left: 0, Top: 0, PacketIndex: []int{0}, Color: rgb.Color{Red: 0, Green: 255, Blue: 255, Brightness: 1}},
					},
				},
			},
		}
...
// writeKrakenColor sends a static RGB color to the Kraken headset
func (d *Device) writeKrakenColor(r, g, b byte) {
	d.mutex.Lock()
	defer d.mutex.Unlock()

	err := razer.KrakenSetStaticColor(d.dev, d.krakenCfg, r, g, b)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write color to Kraken device")
	}
}
	...
	if d.DeviceProfile.RGBProfile == "stand" {
		for _, rows := range d.DeviceProfile.Stand.Row {
			for _, keys := range rows.Zones {
				...
				d.writeKrakenColor(byte(profileColor.Red), byte(profileColor.Green), byte(profileColor.Blue))
			}
		}
		return
	}
	...
	go func(lightChannels int) {
		...
		for {
			...
			default:
				...
				if len(buff) >= 3 {
					d.writeKrakenColor(buff[0], buff[1], buff[2])
				}
...
func (d *Device) writeColorCluster(data []byte, _ int) {
	...
	if len(data) >= 3 {
		d.writeKrakenColor(data[0], data[1], data[2])
	}
}
